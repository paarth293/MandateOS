    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-600 transition"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5 shadow-sm">
          ...
          <p className="text-2xl font-bold text-white mt-2">Ed25519</p>
        ...
        <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5 shadow-sm">
          ...
          <p className="text-2xl font-bold text-white mt-2">300s + Nonce</p>
      ...
      <div className="rounded-xl border border-white/10 bg-slate-900/60 shadow-sm overflow-hidden">
        <div className="border-b border-white/10 bg-slate-900/60/[0.03] px-6 py-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-slate-100">Provisioned Agent Mandates</h2>
          <button
            type="button"
              ...
              return (
                <div
                  ...
                  className={`p-6 transition-colors ${
                    isRevoked ? "bg-slate-900/60/[0.03] opacity-75" : "hover:bg-slate-900/60/[0.02]"
                  }`}
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        <h3 className="text-base font-semibold text-white">{m.agentName}</h3>
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                            m.status === "ACTIVE"
                              ...
                              : m.status === "REVOKED"
                                ...
                                : "bg-slate-900/60/[0.06] text-slate-200"
                          }`}
                        >
                          {m.status}
                        </span>
                    ...
                    <div className="flex items-center gap-2">
                      {!isRevoked && (
                        <button
                          ...
                          className="rounded-md border border-rose-500/20 bg-rose-500/10 px-3 py-1.5 text-xs font-semibold text-rose-400 hover:bg-rose-500/15 transition"
                        >
                          Revoke Mandate
                        </button>
                      )}
                  ...
                  {/* Limit Grid */}
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-4 gap-4 rounded-lg bg-slate-900/60/[0.03] p-4 border border-white/10">
                    <div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {m.allowedCategories.map((cat) => (
                        <span
                          key={cat}
                          className="rounded bg-indigo-500/10 px-2 py-0.5 text-[11px] font-medium text-indigo-300 border border-indigo-500/20"
                        >
                          {cat}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center gap-2 font-mono text-[11px] text-slate-500">
                      ...
                      <span className="bg-slate-900/60 px-1.5 py-0.5 rounded border border-white/10">
                        {shortPub}
                      </span>
                      <button
                        type="button"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-xl rounded-xl bg-slate-900/60 p-6 shadow-2xl border border-white/10 max-h-[90vh] overflow-y-auto">
            ...
            {actionError && (
              <div className="mt-4 rounded-lg bg-rose-500/10 p-3 text-xs text-rose-400 border border-rose-500/20 flex items-center gap-2">
            ...
            {issuedCredentials ? (
              <div className="mt-4 space-y-4">
                <div className="rounded-lg bg-emerald-500/10 p-4 border border-emerald-500/20 text-xs text-emerald-400">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      readOnly
                      value={newMandateId || ""}
                      className="w-full rounded border border-white/10 bg-slate-900/60/[0.03] p-2 font-mono text-xs text-slate-100"
                    />
                    <button
                      type="button"
                      onClick={() => handleCopy(newMandateId || "", "mandate-id")}
                      className="rounded border border-white/10 p-2 hover:bg-slate-900/60/[0.06]"
                    >
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-full rounded border border-white/10 bg-amber-500/10 p-2 font-mono text-xs text-amber-400 font-bold"
                    />
                    <button
                      type="button"
              ...
              <form onSubmit={handleCreateMandate} className="mt-4 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-100 focus:border-indigo-400 focus:outline-none"
                  />
                </div>
                ...
                <div className="pt-2 flex justify-end gap-2">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="rounded-lg border border-white/10 px-4 py-2 text-sm font-semibold text-slate-300 hover:bg-slate-900/60/[0.03] transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={creating}
                    className="rounded-lg bg-indigo-500 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-600 transition disabled:opacity-50"
                  >
